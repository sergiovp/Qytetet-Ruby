...
module ModeloQytetet
  class Jugador
    ...
    def crear_especulador(jugador)
      new(jugador.encarcelado, jugador.nombre, jugador.saldo, jugador.casilla_actual, 
          jugador.factor_especulador, jugador.carta_libertad, jugador.propiedades)
    end
    
    public_class_method :new
=begin
FALTA EL CONSTUCTOR RARO DE JUGADOR
=end
    ...
    def actualizar_posicion(casilla)
      tengo_un_propietario = false
      
      modificar_saldo(Qytetet.SALDO_SALIDA) if casilla.numero_casilla < @casilla_actual.numero_casilla
      
      set_casilla_actual(casilla)
      
      if casilla.soy_edificable
        tengo_un_propietario = casilla.tengo_propietario
        
        if tengo_un_propietario
          @encarcelado = casilla.propietario_encarcelado
        end
        
        if !@encarcelado
          coste_de_alquiler = casilla.cobrar_alquiler
          modificar_saldo(-coste_de_alquiler)
        end
      end
      
      if casilla.tipo == TipoCasilla::IMPUESTO
        coste_impuesto = casilla.coste
        pagar_impuestos(coste_impuesto)
      end
      
      tengo_un_propietario
    end
    ...
    protected :pagar_impuestos, :actualizar_posicion, :convertirme
...
=begin
        ...
        if(es_mia)
            ...
            tengo_saldoo = tengo_saldo(costee)
        end
